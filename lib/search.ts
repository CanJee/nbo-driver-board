...
/** Which driver field a search query matched. Visible fields (name/car) are
 *  checked first so the card needs no extra explanation; phone/notes are
 *  hidden on collapsed cards, so a hit there drives a "why it matched" snippet.
 *  walkie_number is deliberately not searchable — the cards no longer show it,
 *  so a hit there would highlight a card with nothing on it to explain why. */
export type SearchMatchField = 'name' | 'car' | 'phone' | 'notes';
 ...
 *  requires 3+ digits so short numeric queries like car "12" don't light up
